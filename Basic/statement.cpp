...
void StatementType::init() {
  ...
  add("END", {}, [](const std::smatch &data, EvalState &state, Program &program) {
    program.setCurrentLine(-1);
  }, 1);
  add("GOTO", {NUM}, [](const std::smatch &data, EvalState &state, Program &program) {
    program.setCurrentLine(std::stoi(data[1]));
  }, 1);
  add("IF", {EXP, CMP, EXP, THEN, NUM}, [](const std::smatch &data, EvalState &state, Program &program) {
    int lhs = parseExp(data[1])->eval(state);
    int rhs = parseExp(data[3])->eval(state);
    char c = data[2].str()[0];
    bool flag = c == '=' ? lhs == rhs : c == '<' ? lhs < rhs : lhs > rhs;
    if(flag) {
      program.setCurrentLine(std::stoi(data[4]));
    }
  }, 1);
  add("RUN", {}, [](const std::smatch &data, EvalState &state, Program &program) {
    program.run(state);
  }, -1);
  add("LIST", {}, [](const std::smatch &data, EvalState &state, Program &program) {
    program.print();
  }, -1);
  add("CLEAR", {}, [](const std::smatch &data, EvalState &state, Program &program) {
    program.clear();
    state.Clear();
  }, -1);
  add("QUIT", {}, [](const std::smatch &data, EvalState &state, Program &program) {
    exit(0);
  }, -1);
  add("HELP", {}, [](const std::smatch &data, EvalState &state, Program &program) {
